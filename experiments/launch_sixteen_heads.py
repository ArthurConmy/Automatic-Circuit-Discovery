...
from acdc.acdc_utils import cleanup, ct, kl_divergence, make_nd_dict, reset_network, shuffle_tensor
...
from subnetwork_probing.train import do_random_resample_caching, do_zero_caching, iterative_correspondence_from_mask
...
from subnetwork_probing.transformer_lens.transformer_lens.HookedTransformer import (
    HookedTransformer as SPHookedTransformer,
)
from subnetwork_probing.transformer_lens.transformer_lens.HookedTransformerConfig import (
    HookedTransformerConfig as SPHookedTransformerConfig,
)
...
parser.add_argument(
    "--wandb-entity", type=str, required=False, default="remix_school-of-rock", help="Value for WANDB_ENTITY_NAME"
)
...
if get_ipython() is not None:  # heheh get around this failing in notebooks
    args = parser.parse_args(
        [
            line.strip()
            for line in r"""--task=tracr-proportion \
--wandb-mode=offline \
--wandb-dir=/tmp/wandb \
...
if args.task == "ioi":
    ...
    things = get_all_tracr_things(
        task="reverse", metric_name=args.metric, num_examples=num_examples, device=args.device
    )
elif args.task == "tracr-proportion":
    num_examples = 50
    things = get_all_tracr_things(
        task="proportion", metric_name=args.metric, num_examples=num_examples, device=args.device
    )
    ...
    things = get_all_induction_things(
        num_examples=num_examples, seq_len=seq_len, device=args.device, metric=args.metric
    )
    ...
    for n, c in model.named_children():
        if isinstance(c, MaskedHookPoint):
            setattr(
                model,
                n,
                SimpleMaskedHookPoint(mask_shape=c.mask_scores.shape, name=c.name, is_mlp=c.is_mlp).to(args.device),
            )
        else:
...
prune_scores = {
    n: torch.zeros_like(c.mask_scores) for n, c in model.named_modules() if isinstance(c, SimpleMaskedHookPoint)
}
...
for layer_i in range(model.cfg.n_layers):
    keys = [f"blocks.{layer_i}.attn.hook_{qkv}" for qkv in ["q", "k", "v"]] + [f"blocks.{layer_i}.hook_mlp_out"]
    ...
    for qkv in ["q", "k", "v"]:
        for head_i in range(model.cfg.n_heads):
            ...
            nodes = [
                TLACDCInterpNode(name, TorchIndex((None, None, head_i)), incoming_edge_type=EdgeType.ADDITION),
                TLACDCInterpNode(
                    f"blocks.{layer_i}.hook_{qkv}_input",
                    TorchIndex((None, None, head_i)),
...
# %%
serializable_nodes_names_indices = [
    (list(map(str, nodes)), name, repr(idx), prune_scores[name][idx].item()) for nodes, name, idx in nodes_names_indices
]
...
for nodes, hook_name, idx in tqdm.tqdm(nodes_names_indices):
    corr, head_parents = iterative_correspondence_from_mask(
        model, nodes_to_mask=nodes, use_pos_embed=False, newv=False, corr=corr, head_parents=head_parents
    )
