...
class TLACDCCorrespondence:
    ...
    @classmethod
    def setup_from_model(cls, model, use_pos_embed=False, use_split_qkv=True, device=None, sp: Optional[Literal["edge", "node"]] = None):
        correspondence = cls()
            ...
            if not model.cfg.attn_only: 
                ...
                for residual_stream_node in downstream_residual_nodes:
                    correspondence.add_edge(
                        ...
                        edge=Edge(sp=sp, edge_type=EdgeType.ADDITION, device=device),
                        safe=False,
                    )
                ...
                correspondence.add_edge(
                    ...
                    child_node=cur_mlp,
                    edge=Edge(sp=sp, edge_type=EdgeType.PLACEHOLDER, device=device), # EDIT: previously, this was a DIRECT_COMPUTATION edge, but that leads to overcounting of MLP edges (I think)
                    safe=False,
                )
                ...
                if use_split_qkv:
                    for letter in "qkv":
                        ...
                        correspondence.add_edge(
                            ...
                            child_node = cur_head,
                            edge = Edge(sp=sp, edge_type=EdgeType.PLACEHOLDER, device=device),
                            safe = False,
                        )
                        ...
                        correspondence.add_edge(
                            ...
                            edge=Edge(sp=sp, edge_type=EdgeType.DIRECT_COMPUTATION, device=device),
                            safe=False,
                        )
